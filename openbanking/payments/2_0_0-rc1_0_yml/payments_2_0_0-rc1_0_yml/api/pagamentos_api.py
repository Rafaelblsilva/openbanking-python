...
from payments_2_0_0-rc1_0_yml.api_client import ApiClient
...
class PagamentosApi(object):
    ...
    def payments_patch_pix_payments_payment_id(self, body, authorization, payment_id, **kwargs):  # noqa: E501
        ...
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.payments_patch_pix_payments_payment_id(body, authorization, payment_id, async_req=True)
        >>> result = thread.get()
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.payments_patch_pix_payments_payment_id_with_http_info(body, authorization, payment_id, **kwargs)  # noqa: E501
        else:
            (data) = self.payments_patch_pix_payments_payment_id_with_http_info(body, authorization, payment_id, **kwargs)  # noqa: E501
            return data

    def payments_patch_pix_payments_payment_id_with_http_info(self, body, authorization, payment_id, **kwargs):  # noqa: E501
        """Cancelar iniciação de pagamento.  # noqa: E501

         Esse endpoint deve ser usado para cancelar, a pedido do cliente pagador, as transações que estejam em uma das seguintes situações: Agendada com sucesso (SCHD), retida para análise (PDNG) ou aguardando autorização de múltiplas alçadas (PATC).    - Caso a requisição seja bem sucedida, a transação vai para a situação CANC.  - Caso o status do pagamento seja diferente de SCHD/PDNG/PATC ou alguma outra regra de negócio impeça o cancelamento, a requisição PATCH retorna HTTP Status 422 com o code PAGAMENTO_NAO_PERMITE_CANCELAMENTO.  - Caso receba um 422, a iniciadora deve fazer uma requisição GET no pagamento para verificar a situação atual dele, bem como detalhes associados.    O cancelamento de pagamento deve ser enviado até 23:59 (Horário de Brasília) do dia anterior à data de efetivação do pagamento.     # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.payments_patch_pix_payments_payment_id_with_http_info(body, authorization, payment_id, async_req=True)
        >>> result = thread.get()
        ...
        all_params = ['body', 'authorization', 'payment_id', 'x_fapi_auth_date', 'x_fapi_customer_ip_address', 'x_fapi_interaction_id', 'x_customer_user_agent']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

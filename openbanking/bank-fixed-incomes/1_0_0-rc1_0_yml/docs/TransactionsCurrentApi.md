...
api_instance = bank-fixed-incomes_1_0_0-rc1_0_yml.TransactionsCurrentApi(bank-fixed-incomes_1_0_0-rc1_0_yml.ApiClient(configuration))
...
try:
    # Obtém as movimentações recentes da operação de Fundos de Investimento identificada por investmentId. O período a ser considerado para apresentação de movimentações será de até 7 dias - 7 dias anteriores da consulta, incluindo o dia da consulta (D-6).
    api_response = api_instance.bankt_fixed_incomes_get_investments_investment_id_transactions_current(investment_id, authorization, x_fapi_interaction_id, x_fapi_auth_date=x_fapi_auth_date, x_fapi_customer_ip_address=x_fapi_customer_ip_address, x_customer_user_agent=x_customer_user_agent, page=page, page_size=page_size, pagination_key=pagination_key, from_transaction_date=from_transaction_date, to_transaction_date=to_transaction_date)
    pprint(api_response)
except ApiException as e:
    print("Exception when calling TransactionsCurrentApi->bankt_fixed_incomes_get_investments_investment_id_transactions_current: %s\n" % e)
```
